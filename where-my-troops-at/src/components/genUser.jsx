...
function GenUser() {
  ...
  function getDistinctSystems(qualsArray) {
    ...
    qualsArray.forEach((qual) => {
      if (!systemsMap[qual.systemId]) {
        ...
        systemsMap[qual.systemId].is_current =
          systemsMap[qual.systemId].is_current && qual.is_current;
  ...
  const isReady =
    distinctSystems.every((system) => system.is_current) &&
  ...
  function meetsRequirements(mission, certsArray) {
    return mission.requiredCerts.every((required) =>
      certsArray.some(
        (cert) => cert.certification === required && cert.is_current,
      ),
    );
  }
  ...
  const modalContent =
    openModal && ['systems', 'quals', 'certs'].includes(openModal.type)
      ...
      <div className="genUser-dashboard">
        ...
        <div className="user-info user-panel">
          <div className="header">
            <h3>User Info</h3>
            <p id="user-status">{isReady ? 'FMC' : 'NMC'}</p>
            {/**pmc fmc nmc    Not Mission  */}
          </div>
          <p id="rank-name">
            {person.rank} | {person.last_name}, {person.first_name}
          </p>
          <div id="domain-unit-crew">
            Cyber | Unit | <button className="btn-card">Alpha</button>
          </div>
          ...
          <div className="certs">
            ...
            <p className="user-cert-ready numX-display">
              {distinctSystems.filter((system) => system.is_current).length}
            </p>
            <p className="user-cert-inprogress numX-display">0</p>
            <p className="user-cert-notstarted numX-display">
              {distinctSystems.filter((system) => !system.is_current).length}
            </p>
            ...
            <button
              ...
              onClick={() =>
                setOpenModal(
                  openModal?.type === 'quals' ? null : { type: 'quals', quals },
                )
              }
            >
              Crew Quals
            ...
            <p className="user-cert-ready numX-display">
              {quals.filter((qual) => qual.is_current).length}
            </p>
            <p className="user-cert-inprogress numX-display">0</p>
            <p className="user-cert-notstarted numX-display">
              {quals.filter((qual) => !qual.is_current).length}
            ...
            <button
              ...
              onClick={() =>
                setOpenModal(
                  openModal?.type === 'certs' ? null : { type: 'certs', certs },
                )
              }
            >
              Certifications
            ...
            <p className="user-cert-ready numX-display">
              {certs.filter((cert) => cert.is_current).length}
            </p>
            <p className="user-cert-inprogress numX-display">0</p>
            <p className="user-cert-notstarted numX-display">
              {certs.filter((cert) => !cert.is_current).length}
                  ...
                  onClick={() =>
                    setOpenModal(
                      openModal?.type === 'mission' &&
                        openModal.mission.id === mission.id
                        ? null
          ...
          >
            ...
            >
              ...
              {modalContent.items.map((item, index) => (
                <p
                  key={index}
                  className={item.isCurrent ? '' : 'mission-not-ready'}
                >
