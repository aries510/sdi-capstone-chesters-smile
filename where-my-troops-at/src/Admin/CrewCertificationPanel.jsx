...
export default function CrewCertificationPanel() {
  ...
  const linkedCertIdsForRole = (roleId) =>
    new Set(
      rows
        .filter((r) => Number(r.roleId) === Number(roleId))
        .map((r) => r.certId),
    );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(
          capitalize(data.error || 'Failed to link certification.'),
        );
      }
  ...
  const handleDelete = async (row) => {
    ...
    try {
      const res = await fetch(
        `${SERVER_URL}/crewcerts/${row.roleId}/${row.certId}`,
        {
          method: 'DELETE',
        },
      );
      ...
      setRows((prev) =>
        prev.filter(
          (r) => !(r.roleId === row.roleId && r.certId === row.certId),
        ),
      );
    } catch (err) {
      alert(err.message);
      ...
      <ul className="panel-list">
        {rows.length === 0 ? (
          <li style={{ padding: '8px 0' }}>
            No crew certification requirements found.
          </li>
        ) : (
          rows.map((row) => (
            <li
