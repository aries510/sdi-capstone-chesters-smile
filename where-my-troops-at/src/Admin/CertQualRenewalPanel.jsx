...
function CertQualRenewalPanel() {
    ...
    useEffect(() => {
        ...
        Promise.all([qualsPromise, certsPromise]).then(([qualsData, certsData]) => {
            ...
            if (Array.isArray(qualsData)) {
                qualsData.forEach(person => {
                    ...
                    person.qualifications.forEach(qual => {
                        ...
                        if (expiry <= soon) {
                            const entry = getOrCreatePerson(person.personId, person.rank, person.member);
                            entry.quals.push({
            ...
            const grouped = Array.from(byPerson.values())
                .map(person => {
                    person.quals.sort((a, b) => new Date(a.expiryDate) - new Date(b.expiryDate));
                    person.certs.sort((a, b) => new Date(a.expiryDate) - new Date(b.expiryDate));

                    const allDates = [...person.quals, ...person.certs].map(i => i.expiryDate);
    ...
    const badgeStyle = (isExpired) => ({
        ...
        fontWeight: 600,
        marginLeft: '6px',
        backgroundColor: isExpired ? 'rgba(220, 38, 38, 0.15)' : 'rgba(245, 158, 11, 0.15)',
    ...
    const renderSubList = (title, items) => {
        ...
        return (
            <div style={{ marginTop: '6px' }}>
                <span style={{ fontSize: '0.75rem', fontWeight: 'bold', opacity: 0.7 }}>{title}</span>
                    ...
                    {items.map(item => (
                        ...
                        >
                            ...
                            <span style={badgeStyle(item.isExpired)}>
                                {item.isExpired ? 'Expired' : 'Expiring Soon'}: {item.expiryDate}
                            </span>
                        </li>
                    ))}
                </ul>
    ...
    return (
        <div className="trainees-panel">
            <div className="trainees-header">
                <h3>Action Required: Expired & Upcoming Quals/Certs</h3>
            </div>
            {renewals.length === 0 ? (
                <p className="no-trainees-msg">
                    No qualifications or certifications expired or expiring in the next 30 days.
                </p>
            ) : (
                <ul className="trainees-list">
                    {renewals.map(person => (
                        <li key={person.personId} style={{ flexDirection: 'column', alignItems: 'stretch' }}>
